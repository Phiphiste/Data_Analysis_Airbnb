...
create_leaflet <- function(city_name, data_date) {
  ...
  df <-
    subset(
      as.data.frame(read.csv(
        paste(city_name, data_date, "listings.csv", sep = "_")
      ))
      ,
      select = c(
        "id"
  ...
  markers <-
    subset(df, select = c("id", "price", "latitude", "longitude"))
  ...
  return(
    markers %>%
      ...
      addMarkers(
        ~ longitude,
        ~ latitude,
        popup = ~ htmlEscape(paste("ID is:", id, "Price is: $", price)),
...
analysis1 <-
  function(city_name1,
    ...
    cities_df <- rbind(as.data.frame(read.csv(
      paste(
        city_name1,
        find_data_dates(city_name1, data_date1),
        "listings.csv",
        sep = "_"
      )
    )),
    as.data.frame(read.csv(
      paste(
        city_name2,
        find_data_dates(city_name2, data_date2),
        "listings.csv",
        sep = "_"
      )
    )))
    ...
    if (plot_select_a1 == "box") {
      return(
        ggplot(
          cities_df,
          aes_string(x = "city", y = feature_select, fill = "city")
        ) +
          geom_boxplot() +
          ggtitle(
            paste(feature_select, "over Cities (Average value in red)", sep = " ")
          ) +
    ...
    } else if (plot_select_a1 == "bar") {
      ggplot(
        subset(
          cities_df,
          estimated_revenue_30 < 3000,
          c(city, estimated_revenue_30, room_type)
        ),
        aes(
          x = room_type ,
          y = estimated_revenue_30 ,
          fill = as.factor(room_type)
        )
      ) +
...
analysis2 <- function(city_name, data_date, feature_select) {
  ...
  city_df <- as.data.frame(read.csv(paste(
    city_name,
    find_data_dates(city_name, data_date),
    "listings.csv",
    sep = "_"
  )))
  ...
  if (feature_select == "room_type") {
    return(
      ggplot(subset(city_df, city == city_name, room_type)) +
        geom_bar(mapping = aes(
          x = room_type, fill = as.factor(room_type)
        )) +
        ggtitle(paste(
          "Distribution of room types for", city_name
        )) +
    ...
    ggplot(
      subset(city_df, city == city_name , c(room_type, price)),
      aes(
        x = room_type ,
        y = price ,
        fill = as.factor(room_type)
    ...
    ggplot(
      subset(city_df, city == city_name, c(room_type, availability_30)),
      aes(
        x = room_type ,
        y = availability_30 ,
        fill = as.factor(room_type)
      ...
      xlab("Room Type") + ylab("Availability over 30 Days") + labs(fill =
                                                                     "Room Type") +
