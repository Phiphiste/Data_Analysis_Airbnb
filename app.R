...
cities_and_dates <-
    c("munich", "madrid", "amsterdam", "berlin", "antwerp")
feature_list <- c(
    "id",
...
ui <- fluidPage(titlePanel("Airbnb Data Analysis"),
                tabsetPanel(
                    tabPanel(
                        "Analysis 1",
                        ...
                        sidebarLayout(
                            sidebarPanel(
                                selectInput(
                                    ...
                                    choices = cities_and_dates,
                                    selected = "amsterdam"
                                ),
                                radioButtons(
                                    "date_a1_city2",
                                    label = ("Select The dataset from last 3 available"),
                                    inline = TRUE,
                                    choices = list(
                                        "Most Recent" = 1,
                                        "2nd Recent" = 2,
                                ...
                                selectInput(
                                    "feature_select",
                                    "Select the feature you want to cover",
                    ...
                    tabPanel(
                        ...
                        sidebarLayout(
                            sidebarPanel(
                                ...
                                radioButtons(
                                    "date_select2",
                                    label = h3("Select The dataset from last 3 available"),
                                    choices = list(
                                        "Most Recent" = 1,
                                        "2nd Recent" = 2,
                                        "3rd Recent" = 3
                                ...
                                selectInput(
                                    "feature_select_a2",
                                    "Select the feature you want to cover",
                                    choices = list("price", "room_type", "bedrooms", "availability_30"),
                                    selected = "price"
                                )
                            ),
                            mainPanel(
                    ...
                    tabPanel(
                        ...
                        sidebarLayout(
                            ...
                            mainPanel(uiOutput("selections_map"),
                                      leafletOutput("mymap"))
                        )
                    )
...
server <- function(input, output) {
    ...
    output$mymap <- renderLeaflet({
        create_leaflet(input$city_select,
                       find_data_dates(input$city_select, input$date_select))
    })
